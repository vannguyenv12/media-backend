...
export class Get {
  public async reactions(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.CREATED).json({ message: 'Post reactions', reactions: reactions[0], count: reactions[1] });
  }
  ...
  public async singleReactionByUsername(req: Request, res: Response): Promise<void> {
    ...
    res
      ...
      .json({ message: 'Single post reactions by username', reactions: reactions.length ? reactions[0] : [], count: reactions[1] });
  }
  ...
  public async reactionsByUsername(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.CREATED).json({ message: 'All reactions by username', reactions });
  }
}
