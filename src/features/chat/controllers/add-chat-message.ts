...
export class Add {
  ...
  public async message(req: Request, res: Response): Promise<void> {
    ...
    if (selectedImage.length) {
      ...
      fileUrl = `https://res.cloudinary.com/dfapum2fd/image/cloud/v${result.version}/${result.public_id}`;
    }
  ...
  public async removeChatUsers(req: Request, res: Response): Promise<void> {
    const chatUsers = await messageCache.removeChatUserToCache(req.body);
    socketIOChatObject.emit('add chat users', chatUsers);
  ...
  private emitSocketIOEvent(data: IMessageData) {
    socketIOChatObject.emit('message receive', data);
  ...
  private async messageNotification({ currentUser, message, receiverName, receiverId }: IMessageNotification): Promise<void> {
    ...
    if (cachedUser.notifications.messages) {
      const templateParams: INotificationTemplate = {
        ...
        message,
        header: `Message Notification from ${currentUser.userId}`
      };
